    ...
    return (<div key={id}>
      <div className="box-border w-[300px] h-[470px] rounded-lg shadow-xl shadow-gray-400 transition duration-300 ease-in  hover:transition hover:duration-300 hover:ease-out">
        ...
        <div className="flex flex-col bg-slate-100 h-[200px] rounded-br-lg rounded-bl-lg px-3 justify-start gap-y-[2px] relative">
          ...
          <Link to={`/product/${id}`}>
            <button
              className="absolute top-24 w-[270px] border-2 border-black h-10 rounded transition duration-500 ease-in font-bold hover:transition hover:duration-500 hover:ease-out hover:bg-stone-900 hover:text-white"
            >View Product</button>
          </Link>
          <button
            className={inStock ? "absolute top-36 bg-yellow-400 text-white w-32 h-10 rounded hover:bg-yellow-300 transition duration-500 ease-in hover:transition hover:duration-500 hover:ease-out" : "absolute top-36 bg-white text-red-600 w-32 h-10 rounded disabled:cursor-not-allowed border-2 border-red-600"}
            disabled={!inStock}
